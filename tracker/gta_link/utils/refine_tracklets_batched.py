...
def process_batch_worker(args):
    ...
    batch_tracklets, merge_dist_thres, max_x_range, max_y_range, batch_idx = args
    logger.info(f"Processing batch {batch_idx} with {len(batch_tracklets)} tracklets")
    merged_batch = merge_tracklets(batch_tracklets, merge_dist_thres, max_x_range, max_y_range)
    logger.info(f"Batch {batch_idx}: {len(merged_batch)} tracklets remaining")
    return merged_batch


def merge_tracklets_batched_parallel_processes(tracklets, seq2Dist, batch_size=50, seq_name=None, 
                                             max_x_range=None, max_y_range=None, merge_dist_thres=None, 
                                             max_workers=None):
    """
    Parallel version using ProcessPoolExecutor for CPU-intensive batch processing.
    Good for CPU-bound operations but requires more memory.
    """
    if max_workers is None:
        max_workers = min(mp.cpu_count(), 4)  # Limit to avoid memory issues
    
    temp_tracklets = {}
    tracklet_items = list(tracklets.items())
    
    tracklet_count = len(tracklet_items)
    batch_size = tracklet_count // max_workers
    # Prepare batch arguments
    batch_args = []
    for i in range(0, tracklet_count, batch_size):
        batch_tracklets = dict(tracklet_items[i:i+batch_size])
        batch_args.append((batch_tracklets, merge_dist_thres, max_x_range, max_y_range, i))
    
    logger.info(f"Processing {len(batch_args)} batches with {max_workers} workers")
    
    # Process batches in parallel
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        results = list(executor.map(process_batch_worker, batch_args))
    
    # Gather all results
    for merged_batch_tracklets in results:
        temp_tracklets.update(merged_batch_tracklets)
    
    logger.info(f"Merging {len(temp_tracklets)} tracklets after parallel processing.")
    merged_tracklets = merge_tracklets(temp_tracklets, merge_dist_thres, max_x_range, max_y_range)
